...
void NvgWindow::drawSpeed(QPainter &p) {
  ...
  configFont(p, "Open Sans", 276, "Bold");
  drawText(p, rect().center().x(), 230 + 540, speed);  // 
  //configFont(p, "Open Sans", 56, "Regular");
  //drawText(p, rect().center().x(), 310 + 600, s->scene.is_metric ? "km/h" : "mph", 200);  // 
}
...
void NvgWindow::drawSpeedLimit(QPainter &p) {
  ...
  if(activeNDA > 0)
  {
      int w = 120;
      int h = 54;
      int x = (width() + (bdr_s*2))/2 - w/2 - bdr_s - 700; //NDA 
